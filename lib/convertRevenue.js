...
function convertRevenue(reportDir) {
  ...
  walker.on('end', () => {
    ...
    if (global.missingTitles.length > 0) {
      // fs.writeFileSync('missing.txt', JSON.stringify(lodash.uniq(global.missingTitles), null, 2))
      const missingTitlesReport = lodash.map(global.missingTitles, l => `  '${l}': '',`)
      console.log('Missing Titles:')
      fs.writeFileSync('missing.txt', '{\n')
      lodash.forEach(lodash.uniq(missingTitlesReport), l => {
        console.log(l)
        fs.appendFileSync('missing.txt', `${l}\n`)
      })
      fs.appendFileSync('missing.txt', '}\n')
    }
  })
