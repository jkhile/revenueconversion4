...
function _mapFields(row) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Add RefNumber here because it depends on formatSpecific mapped fields and the inputFile name
      formatMappings.RefNumber = `${path.parse(self.inputFile).name}_${formatMappings.CurrencyCode}`
      ...
      currencyConverter.getRate(fieldMap['CurrencyCode'], 'USD').then((rate) => {
        if (fieldMap['CurrencyCode'] !== 'USD') {
          fieldMap.Note = `Converted to USD from ${fieldMap.LineAmount.toFixed(2)} ${fieldMap[
            'CurrencyCode'
          ]} at ${rate}`
...
function _nookSpecific(row, transactionDate) {
  if (R.has('Unit Royalty', row)) {
    ...
    return {
      Customer: 'Barnes & Noble - Nook',
      LineItem: 'Other Income - Nook',
      LineQty: parseInt(row['Net Units Sold']),
      LineDesc: `Compensation from Nook Sales ${transactionDate.format('YYYY-MMM')}`,
      LineUnitPrice: parseFloat(row['Royalty per Unit']),
      LineAmount: parseFloat(row['Total Royalty']),
      Note: '',
      Title: row['Title'],
