...
function _lsiSpecific(row, transactionDate) {
  ...
  const periodColumnPrefix = row.hasOwnProperty('MTD_avg_wholesale_price')
    ? 'MTD_'
    : 'PTD_'
  const avgWholesalePrice = parseFloat(row[`${periodColumnPrefix}avg_wholesale_price`])
  const printCharge = parseFloat(row[`${periodColumnPrefix}extended_print_charge`])
