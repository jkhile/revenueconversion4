...
function convertRevenue(reportDir) {
  ...
  walker.on('file', (root, filestats, next) => {
    ...
    // }
    if (
      // we're looking for files that match 'yyyy-MM_{...}.ext'
      // and that don't match the same with 'TPI{...}.csv' at the end
      ...
      if (matched.length > 1) {
        format = matched[1]
        fullFormat = matched[2] ? `${format}${matched[2]}` : format
      }
      if (!format) {
      ...
      _fileConverted(inputFile).then((converted) => {
        if (!converted) {
          convertRevenueFile(format, fullFormat, inputFile)
            .then(() => {
              next()
            })
            .catch((error) => {
