...
const { _loadAcxXlsx } = require('./_loadAcxXlsx')
...
function convertRevenueFile(format, fullFormat, inputFile) {
  return new Promise((resolve, reject) => {
    ...
    if (format === 'kindle') {
      ...
      loadInputFile = _loadAcxXlsx(inputFile, format)
    } else if (format === 'kobo') {
    ...
    loadInputFile
      .then((revenueData) => {
        ...
        const mapFileName = path.join(
          revenueDir,
          `${company}-title-to-QB-class-map.js`
        )
      ...
      .then((convertedRows) => {
        ...
        const mergedRows = convertedRows.reduce((accumulator, row) => {
          ...
          const found = lodash.find(accumulator, (value) => {
            return (
              value.lineItemClass === row.lineItemClass &&
              value.currencyCode === row.currencyCode
            )
          })
          if (found) {
            found.lineItemQuantity += row.lineItemQuantity
            found.lineItemAmount = (
              parseFloat(found.lineItemAmount) + parseFloat(row.lineItemAmount)
            ).toFixed(2)
        ...
        const outputPromises = lodash.map(byCurrency, (rows, currencyCode) => {
          ...
          const outputFile = path.join(
            parts.dir,
            `${parts.name}_${currencyCode}-TPI.csv`
          )
