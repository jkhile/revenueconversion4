...
function _mapFields(row) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      formatMappings.invoiceNo = `${transactionDate.format('YYYY-MM')}_${self.format}_${formatMappings.currencyCode}`
      ...
      currencyConverter.getRate(fieldMap['currencyCode'], 'USD').then((rate) => {
        if (fieldMap['currencyCode'] !== 'USD') {
          fieldMap.memo = `Converted to USD from ${fieldMap.lineItemAmount.toFixed(2)} ${fieldMap[
            'currencyCode'
          ]} at ${rate}`
...
function _koboSpecific(row, transactionDate) {
  ...
  return {
    ...
    lineItemRate: parseFloat((parseFloat(row[headers['lineItemAmount']]) / parseInt(row[headers['quantity']])).toFixed(2)),
